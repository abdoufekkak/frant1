...
@Component({
  selector: 'app-tableaulivreur',
  templateUrl: './tableaulivreur.component.html',
  styleUrls: ['./tableaulivreur.component.scss'],
})
export class TableaulivreurComponent implements OnInit {
  valeur: string = '';
  ...
  disponible(disp: string, livreur: Livreur, id?: number) {
    ...
    //       x="1"
    //     }
    //     this.service.disactiverLivreur(x,id!).pipe(tap(()=>{
    //       livreur.disponible=x;
    //     })).subscribe(e=>console.log(e),err=>console.log(err))
  }

  onValeurChange() {
    // this.service.chercherLivreur(this.valeur).pipe(
